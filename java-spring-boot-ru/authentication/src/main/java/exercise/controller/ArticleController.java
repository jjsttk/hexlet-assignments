...
public class ArticleController {
    ...
    public ArticleDTO create(@RequestBody @Valid ArticleCreateDTO createDTO) {
        var model = articleMapper.map(createDTO);
        model.setAuthor(userUtils.getCurrentUser());
        return articleMapper.map(articleRepository.save(model));
    }
    // END
